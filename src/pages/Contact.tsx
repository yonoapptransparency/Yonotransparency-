  ...
  return (
    <div className="max-w-7xl mx-auto py-16 px-6 sm:px-10 animate-fade-in pb-20">
      ...
      >
        ...
        <form className="lg:col-span-12 xl:col-span-7 bg-white dark:bg-zinc-900 border border-black/5 dark:border-white/5 rounded-[32px] p-8 sm:p-12 shadow-sm space-y-6">
          <h2 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100 pb-2 border-b border-black/5">Send a Message</h2>
          <div>
            <label className="block text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-2">Name</label>
            <input type="text" className="w-full p-4 rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-black/5 dark:border-white/5 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white text-zinc-900 dark:text-zinc-100 font-medium" placeholder="Your name" />
          </div>
          <div>
            <label className="block text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-2">Email</label>
            <input type="email" className="w-full p-4 rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-black/5 dark:border-white/5 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white text-zinc-900 dark:text-zinc-100 font-medium" placeholder="your@email.com" />
          </div>
          <div>
            <label className="block text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-2">Message</label>
            <textarea className="w-full p-4 rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-black/5 dark:border-white/5 h-44 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white text-zinc-900 dark:text-zinc-100 font-medium" placeholder="How can we help?"></textarea>
          </div>
          <button type="submit" onClick={(e) => { e.preventDefault(); alert("Message sent successfully!"); }} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl shadow-md hover:shadow-lg transition-all hover:scale-[1.01] active:scale-[0.99] text-sm">
            Send Message
          </button>
        </form>
      </motion.div>
